...
func (pst *PeerScoreTool) AnalyzeConnectionTiming() ConnectionTiming {
	...
	// Get peers data for accurate fast disconnect counting
	peers := pst.peers
	...
	analysis.SuspiciousPatterns = pst.identifySuspiciousPatterns(analysis, peers)
...
func (pst *PeerScoreTool) identifySuspiciousPatterns(analysis ConnectionTiming, peers map[string]*PeerStats) []string {
	...
	if analysis.FastestDisconnect > 0 && analysis.FastestDisconnect < 5*time.Second {
		fastCount := 0
		// Count actual individual peer disconnects under 5 seconds
		for _, peer := range peers {
			// Check if any goodbye timing for this peer was under 5 seconds
			for _, goodbyeTiming := range peer.GoodbyeTimings {
				if goodbyeTiming.DurationFromStart < 5*time.Second {
					fastCount++
					break // Count each peer only once, even if multiple fast goodbyes
				}
			}
		}
