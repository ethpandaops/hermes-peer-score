...
func (hc *DefaultHermesController) applyValidationConfig(cfg *eth.NodeConfig) {
	...
	case "independent":
		hc.logger.Info("Configuring independent validation mode - Hermes will use Prysm for beacon state but perform validation internally")
	case "delegated":
		hc.logger.Info("Configuring delegated validation mode - Hermes will send validation requests to Prysm for processing")
